...
int sys_process(uint64_t ip, uint8_t priority, uint64_t argc,char argv[5][20], void *customStdin,void *customStdout);
int sys_kill(int pid);
int sys_get_pid();
int sys_block(int pid);
void sys_nice(int pid, int priority);
int sys_create_child(int ppid,uint64_t ip, uint8_t priority, uint64_t argc,char argv[5][20], fd *customStdin,fd *customStdout);
